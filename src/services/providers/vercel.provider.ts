...
// System variables to ignore during sync (auto-generated by Vercel)
...
export const vercelProvider: Provider = {
  ...
  async getTeam(
    ...
    try {
      ...
      logger.warn(
        { teamId, error: error instanceof Error ? error.message : "Unknown" },
  ...
  async listEnvVars(
    ...
    // Filter by target environment and exclude system variables
    const targetEnv = environment.toLowerCase();
    return data.envs
      .filter((env) => env.target.some((t) => t.toLowerCase() === targetEnv))
      .filter((env) => !IGNORED_VAR_PREFIXES.some((prefix) => env.key.startsWith(prefix)))
      .map((env) => ({
        key: env.key,
